...
class AuthController:
    ...
    def login(request: Request, username, password):
        ...
        hotel_date = (
            FOService.get_hotel_date()
        )  # Hàm "SELECT TOP 1 HotelDate..." bạn đã viết
        ...
        # Tạo một danh sách chỉ chứa Code để trả về cho Frontend (cho nhẹ và dễ check JS)
        # Lấy dữ liệu từ session
        raw_perms = request.session.get("permissions", [])
        user_perms = [
            (p['code'].lower() if isinstance(p, dict) else str(p).lower()) 
            for p in raw_perms if p
        ]

        logger.info(
            f"User {username} logged in successfully with {len(permissions)} perms"
        )
    ...
    def admin_reset_pass(data, admin_user: str):
        ...
        try:
            ...
            AuthService.admin_reset_password_logic(
                data.username, data.new_password, admin_user
            )
            return {
                "status": "success",
            ...
            raise HTTPException(
                status_code=500, detail="Lỗi hệ thống khi reset mật khẩu"
            )
